...
function initDatabase() {
  ...
  if (!fs.existsSync(PROJECTS_FILE)) {
    ...
    if (lines.length > 0 && !lines[0].includes('user_id')) {
      ...
      fs.writeFileSync(PROJECTS_FILE, newLines.join('\n'), 'utf8');
        logger.info('项目 CSV 文件已迁移（添加 user_id 字段）');
    }
  }
  ...
  if (!fs.existsSync(EVENTS_FILE)) {
    ...
    if (lines.length > 0) {
      ...
      if (!header.includes('visitor_id')) {
        // 需要迁移：添加新字段到表头
        const newHeader = header.trim() + ',visitor_id,session_id,entry_page,source,search_keyword,visit_duration,page_count,region\n';
        ...
        fs.writeFileSync(EVENTS_FILE, newLines.join('\n'), 'utf8');
        logger.info('事件 CSV 文件已迁移（添加新字段）');
      }
    }
  }

  logger.info('CSV 数据文件初始化完成');
}
